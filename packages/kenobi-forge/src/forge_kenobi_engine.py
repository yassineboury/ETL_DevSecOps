...
class ForgeKenobiEngine:
    ...
    def _load_kenobi_config(self) -> Dict:
        ...
        framework_match = re.search(r'<!-- FRAMEWORK_TAG START -->(.*?)<!-- FRAMEWORK_TAG END -->', content, re.DOTALL)
        if framework_match:
            framework_info = {}
    ...
    def _load_compliance_rules(self) -> List[ComplianceRule]:
        ...
        rules.extend([
            ComplianceRule(
                ...
                description="Fichiers snake_case, colonnes <source>_<metric>_<unit>",
                category="naming",
                severity="ERROR"
            ),
            ComplianceRule(
    ...
    def run_full_compliance_check(self) -> Dict[str, Any]:
        ...
        compliance_report = {
            ...
            "warnings": len([r for r in all_results if r.status == ComplianceStatus.WARNING]),
            "results": [
                {
                    "rule_id": r.rule_id,
...
def main():
    ...
    parser.add_argument("--project-root", type=Path, default=".", help="Racine du projet")
    parser.add_argument("--output", type=Path, help="Fichier de sortie pour le rapport")
    parser.add_argument("--check-only", action="store_true", help="Affichage console uniquement")
    
    args = parser.parse_args()
    
    engine = ForgeKenobiEngine(args.project_root)
    
    if args.check_only:
        report = engine.run_full_compliance_check()
        print(f"🔍 Forge-Kenobi Compliance Check")
