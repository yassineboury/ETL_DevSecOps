...
class ForgeKenobiAssistant:
    ...
    def _load_forge_kenobi_context(self) -> ForgeKenobiContext:
        ...
        kenobi_file = self.project_root / "packages" / "kenobi-forge" / "FORGE-KENOBI.md"
    ...
    def _extract_mission(self, content: str) -> str:
        ...
        if match:
            ...
            obj_match = re.search(r'\*\*OBJECTIF\*\*\s*(.*?)(?=\*\*|$)', mission_content, re.DOTALL)
            if obj_match:
    ...
    def _extract_coding_conventions(self, content: str) -> Dict[str, str]:
        ...
        match = re.search(r'<coding_conventions>(.*?)</coding_conventions>', content, re.DOTALL)
        conventions = {}
        if match:
            conv_content = match.group(1)
    ...
    def _extract_specifications(self, content: str) -> List[Dict]:
        ...
        spec_match = re.search(r'<specification>(.*?)</specification>', content, re.DOTALL)
        specifications = []
        
        if spec_match:
            spec_content = spec_match.group(1)
            # Extraire chaque SPC-XXX
            spc_pattern = r'#### (SPC-[A-Z]+-\d+) – ([^*]+) \*\(active\)\*\s*(.*?)(?=####|$)'
    ...
    def _extract_architecture(self, content: str) -> Dict[str, List[str]]:
        ...
        arch_match = re.search(r'<architecture_and_infra>(.*?)</architecture_and_infra>', content, re.DOTALL)
        architecture = {
            "import_rules": [],
            "required_structure": [
                "etl/core/",
    ...
    def _extract_constraints(self, content: str) -> Dict[str, str]:
        ...
        constraints_match = re.search(r'<constraints>(.*?)</constraints>', content, re.DOTALL)
        constraints = {}
        
        if constraints_match:
            const_content = constraints_match.group(1)
            constraints.update({
                "python_version": "3.12",
    ...
    def before_action_check(self, action_type: ActionType, details: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        if action_type in [ActionType.CREATE_FILE, ActionType.MODIFY_FILE]:
            ...
            if file_path.endswith('.py'):
                ...
                if filename != '__init__' and not re.match(r'^[a-z][a-z0-9_]*[a-z0-9]$', filename):
                    return False, f"❌ SPC-GEN-01: Fichier '{filename}' ne respecte pas snake_case"
        ...
        if action_type == ActionType.CREATE_STRUCTURE:
            ...
            for req_dir in required_dirs:
                if req_dir not in str(details):
                    return False, f"❌ Architecture ETL: Structure '{req_dir}' requise manquante"
        ...
        if action_type == ActionType.RUN_COMMAND:
            ...
            if 'pip install' in command and 'poetry' not in command:
                return False, "❌ Contraintes: Utiliser Poetry ≥ 1.8 pour les dépendances"
        
        return True, f"✅ Action {action_type.value} conforme aux spécifications Forge-Kenobi"
    
    def execute_with_compliance(self, action_type: ActionType, action_details: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        is_compliant, compliance_message = self.before_action_check(action_type, action_details)
        ...
        try:
            ...
            return {
                ...
                "current_pass": self.current_pass,
                "specifications_applied": [spec["id"] for spec in self.context.specifications]
            }
...
{chr(10).join(f"  • {rule}" for rule in self.context.architecture_rules['required_structure'])}
...
def get_forge_kenobi_assistant(project_root: str = ".") -> ForgeKenobiAssistant:
    ...
    """
    return ForgeKenobiAssistant(Path(project_root))
...
if __name__ == "__main__":
    ...
    test_result = assistant.execute_with_compliance(
        ActionType.CREATE_FILE,
        {"file_path": "test_file.py", "content": "# Test"}
    )
